...
When producing interactive HTML dashboards or data visualizations (e.g. sankey/alluvial diagrams, reclassification reports), follow these conventions. Reference implementation: `LADC_flow/results/sankey_217_ladc.html`.
...
For data categories, define `--hi`, `--int`, `--low` (or equivalent semantic names) plus `*-light` variants at 0.08 alpha for background fills. Use the Elahi accent palette (purple/orange/green) for risk tiers.
