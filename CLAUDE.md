...
**Orient data is invisible to the user.** The SessionStart hook injects orient data (vault state, goals, conditions, git status) into the assistant's context. Surface the orient summary when the user runs `/next`, which is the natural entry point for "what's going on?"
